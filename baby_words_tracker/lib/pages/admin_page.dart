...
class _AdminPageState extends State<AdminPage> {
  ...
  Future<void> _getCustomClaims(String email) async {
    late final Map<String, dynamic>? customClaims;
    bool failure = false;
    final targetEmail = _selectedUserEmail.isEmpty ? email : _selectedUserEmail;
    try {
      ...
      if (context.mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('Failed to get custom claims: $e'),
          ),
        );
      } else {
    ...
    if (failure) {
      ...
      if (context.mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('No custom claims found for this user.'),
          ),
        );
      } else {
        debugPrint("No custom claims found for this user.");
      }
      setState(() {
        _userRoles = {};
      });
        ...
        child: Column(
          ...
          children: [
            TextField(
              ...
              onSubmitted: (String value) => _getCustomClaims(value),
            ),
            // ),
            Flexible(
              fit: FlexFit.loose,
                    ...
                    children: [
                      ...
                      _buildPadded(
                        ElevatedButton(
                          ...
                          onPressed: () => _getCustomClaims(_selectedUserEmail),
                        ),
                      ),
                      Expanded(
                        child: SingleChildScrollView(
                          ...
                          child: Column(
                            ...
                            children: [
                              ...
                              _buildPadded(Text("Demo Controls:")),
                              _buildPadded(
                                Padding(
                                  padding: EdgeInsetsGeometry.only(left: 8),
                                  child: Column(
                                    ...
                                    children: [
                                      _buildPadded(
                                        ElevatedButton(
                                          child: const Text("Assign Demo Role"),
                                          onPressed: () async {
                                            if (await showConfirmationDialog(
                                                context,
                                                'Are your sure you want to assign the Demo role to $_selectedUserEmail?')) {
                                              await _callRoleFunction(
                                                  'giveDemoClaim');
                                            }
                                          },
                                      ...
                                      _buildPadded(
                                        ElevatedButton(
                                          child: const Text("Remove Demo Role"),
                                          onPressed: () async {
                                            if (await showConfirmationDialog(
                                                context,
                                                'Are your sure you want to remove the Demo role from $_selectedUserEmail?')) {
                                              await _callRoleFunction(
                                                  'removeDemoClaim');
                                            }
                                          },
